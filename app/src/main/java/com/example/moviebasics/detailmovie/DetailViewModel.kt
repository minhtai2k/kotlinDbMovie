...
import com.example.moviebasics.network.MovieDetailApiService
...
class DetailViewModel @Inject constructor(
    val db: AppDatabase,
    private val retrofitDetailMovie: MovieDetailApiService
) : ViewModel() {
...
//    val isLoading: LiveData<Boolean> = _isLoading
    ...
    fun getMovieDetail(isConnected: Boolean, id: Int) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                if (isConnected) {
                    val data = retrofitDetailMovie.getMovieDetail(movieId = id)
